...
export default function PublicLandingPage() {
  ...
  useEffect(() => {
    // Fetch generic 'public' data
    apiFetch(`/catalog/public/data`)
      .then(res => {
        setData(res);
    ...
    <>
                ...
                <div className="p-7 flex-1 flex flex-col">
                  ...
                  <div className="mt-auto pt-6 border-t border-gray-50 flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-1">Rate</span>
                      <span className="text-xl font-black text-[#0d2e2e]">
                        {p.is_poa ? 'POA' : `$${p.price.toFixed(2)}`}
                      </span>
                    </div>

                    {/* View Details Button instead of Shopping Bag */}
                    <div className="w-14 h-14 rounded-2xl bg-teal-50 text-teal-600 group-hover:bg-[#0d2e2e] group-hover:text-white transition-all duration-500 flex items-center justify-center shadow-sm">
      ...
      {activeProduct && (
        <ProductDetailsModal
          ...
          // Notice: onAddToCart is omitted, so it will be read-only!
